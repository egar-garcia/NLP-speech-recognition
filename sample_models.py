...
def bidirectional_rnn_model(input_dim, units, output_dim=29):
    ...
    bidir_rnn = Bidirectional(
        GRU(units, activation='relu', return_sequences=True, implementation=2),
...
def final_model(input_dim, units, recur_layers, output_dim=29):
    ...
    for i in range(recur_layers):
        simp_rnn = Bidirectional(
            GRU(units, activation='relu', return_sequences=True, implementation=2),
            merge_mode='concat', name='rnn_'+str(i))(last_layer)
        bn_rnn = BatchNormalization(name='bn_rnn_'+str(i))(simp_rnn)
        last_layer = Dropout(rate=0.5, name='dropout_'+str(i))(bn_rnn)
    time_dense = TimeDistributed(Dense(output_dim), name='time_dense')(last_layer)
